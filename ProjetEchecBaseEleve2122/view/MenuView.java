...
import command.choiceStyle.ChoiceColor;
import command.choiceStyle.ChoiceStyle;
import command.compensation.CompensableCommand;
import command.compensation.CompensationConversation;
...
public class MenuView extends MenuBar {
	...
	private CompensationConversation compensationConversation = new CompensationConversation();
		...
		color1.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle (ActionEvent event) {
				...
				colorPicker.setOnAction(colorEvent -> {
					...
					compensationConversation.exec(new ChoiceColor(GuiConfig.whiteSquareColor));
					colorDialog.close();
				});
		...
		color2.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle (ActionEvent event) {
				...
				colorPicker.setOnAction(colorEvent -> {
					...
					compensationConversation.exec(new ChoiceColor(GuiConfig.blackSquareColor));
					colorDialog.close();
				});
	...
	private Menu newMenuStyle () {
		...
		style1.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle (ActionEvent event) {
				...
				compensationConversation.exec(new ChoiceStyle());
